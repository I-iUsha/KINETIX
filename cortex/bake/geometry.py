...
:func:`bake_geometry_parts`' return so downstream bakes can record provenance â€”
...
# path) and is a C extension a thread can't interrupt â€” so we run it in a
...
_COACD_TIMEOUT_S = float(os.environ.get("KINETIX_COACD_TIMEOUT", "20"))
...
def _preprocess(mesh: trimesh.Trimesh) -> bool:
    ...
    for step in (
        ...
        try:
            ...
            pass
    # Record honest source topology after harmless seam cleanup. Hole filling below is
...
def _coacd_worker(q, vertices, faces, threshold, params) -> None:
    """Subprocess entry: run CoACD and push ``[(V, F), â€¦]`` (or ``[]`` on error) to ``q``."""
    try:
        import coacd
...
def _coacd_parts(mesh: trimesh.Trimesh) -> list[trimesh.Trimesh]:
    ...
    subprocess is the only way to bound a C-extension call â€” a thread can't kill it.
    """
    try:
        import coacd  # noqa: F401 â€” probe availability before paying the spawn cost
    except Exception:
        return []
    ...
    try:
        ...
        raw = []  # CoACD overran its budget â†’ fall back to a convex decomposition
    finally:
        if proc.is_alive():
            proc.terminate()
        proc.join(timeout=2)
