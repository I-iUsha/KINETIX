...
# --------------------------------------------------------------------------- #
# asset_id -> UE StaticMesh package path (e.g. "/Game/kinetiXImport/SM_Tree"), used by
...
def _tf(pos: list[float], quat: Optional[list[float]] = None,
    ...
    adapter — so transforms straight out of the Unreal MCP line up with kinetiX's gates."""
...
def ue5_to_canonical(pos: list[float], quat: Optional[list[float]] = None) -> str:
    """Convert an Unreal transform (cm, left-handed, Z-up) → kinetiX canonical (m,
    right-handed, Z-up). Feed the Unreal MCP's location/rotation straight in to mirror a
    placement into kinetiX for validation. Returns JSON ``{"pos":[...], "quat":[x,y,z,w]}``."""
    from conscience.ue5 import adapter
...
def canonical_to_ue5(pos: list[float], quat: Optional[list[float]] = None) -> str:
    """Convert a kinetiX canonical transform (m, right-handed) → Unreal (cm, left-handed).
...
def export_placement_wdf(out_path: Optional[str] = None, asset_ids_json: Optional[str] = None,
    ...
    """Export the learned placement language as a ``.wdf`` document — the portable kinetiX
