"""
Vultr-hosted mask providers â€” self-hosted HF models on a GPU box.
...
configured URL **and** a short-TTL-cached ``/health`` ping â€” these providers grey out in the rail
when the box is down. Config resolves like the HF/Gemini keys: env ``KINETIX_VULTR_URL`` /
``KINETIX_VULTR_TOKEN`` with gitignored ``.vultr_url`` / ``.vultr_token`` repo-root fallbacks.
...
maps to a per-part scalar by the same heightâ†”image-row correspondence.
"""
...
_HEALTH_TTL = 30.0  # seconds â€” cache the health ping so catalog() (called per rail render) stays cheap
...
def _vultr_url() -> str | None:
    return os.environ.get("KINETIX_VULTR_URL") or _read_file(_URL_FILE)


def _vultr_token() -> str | None:
    return os.environ.get("KINETIX_VULTR_TOKEN") or _read_file(_TOKEN_FILE)
...
def _vultr_request(task: str, image_bytes: bytes, params: dict | None = None):
    ...
    try:
        ...
        # The box returns {"detail": "<ErrType>: <msg>"} on failure â€” surface it verbatim so the
...
def _depth(asset, images) -> dict:
    """Monocular depth grid â†’ per-part scalar by the heightâ†”image-row correspondence.

    The box returns a downsampled depth ``grid`` (rows topâ†’bottom of the render) + ``min``/``max``.
