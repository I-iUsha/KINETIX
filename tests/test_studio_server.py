"""Tests for the kinetiX Studio FastAPI backend (studio/server.py).
...
def test_export_repaired_returns_transformed_glb():
    ...
    tf = {"pos": [2.0, 0.0, 0.0], "quat": [0.0, 0.0, 0.0, 1.0], "scale": [1.0, 1.0, 1.0]}
    r = client.post(
        "/export_repaired",
        data={"transform": json.dumps(tf)},
        files={"mesh": ("box.glb", source, "model/gltf-binary")},
    )
    assert r.status_code == 200, r.text
