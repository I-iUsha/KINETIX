# kinetiX ↔ Unreal MCP integration

Goal: an AI agent (Claude) **builds a forest in Unreal** with your existing Unreal MCP
(*hayba*) and **validates every placement** through kinetiX's MCP — collision-free, stable,
...
```
        ┌─────────────── Claude (MCP client) ───────────────┐
        │                                                    │
   hayba MCP                                            kinetix-cortex MCP
  (build/read the UE scene)                       (bake + validate + repair)
        │                                                    │
   Unreal Engine  ◀──────  place / move trees  ──────────────┘
                  ──────  read transforms  ───────▶  mirror into kinetiX, validate
```

kinetiX never writes to Unreal here — hayba owns the scene. kinetiX is the **physics
conscience**: it bakes the tree once, mirrors hayba's placements, and answers
"is this valid? if not, where should it go?"

---

## 1. Register kinetix-cortex alongside hayba

Add kinetiX's stdio MCP server to the same client config that already has hayba
...
Restart the client. You should see both tool sets. kinetiX exposes (forest-relevant):
...
kinetiX validates in **canonical space**: metres, right-handed, Z-up.
...
1. Get the tree mesh as a file kinetiX can read (`.glb`/`.obj`). If it's a UE asset,
...
**Per tree — build with hayba, validate with kinetiX:**
4. hayba: spawn/locate a tree → read its UE transform `(location_cm, rotation)`.
5. `place_asset("tree", "tree_07", pos=<cm>, quat=<…>, space="ue5")` — mirror it into kinetiX.
...
> You have two MCP servers: **hayba** (build/read the Unreal scene) and **kinetix-cortex**
...
That's the demo: an AI-built Unreal forest that kinetiX proves is collision-free, stable,
