# Vultr-hosted masking â€” design spec
...
serves a small curated model set â€” effectively just `segformer-b0` for `part_segmentation`,
...
self-host arbitrary HF models and call them from the studio backend â€” unlocking "more advanced
...
- Auto-provisioning / autoscaling the box (manual start/stop â€” see lifecycle).
- Batching, queueing, multi-user concurrency, persistence on the box (single-user hackathon).
- Changing the existing `hf.py` router provider â€” it stays as a free fallback.
- Frontend changes â€” new masks reuse existing archetypes (`categorical`/`scalar`), so the rail
  and renderers already handle them.

## Architecture (Approach A â€” generic remote inference server + thin provider per model)

```
vultr/                       â† deployed TO the Vultr GPU box (self-contained, not imported by cortex)
...
cortex/masks/
  ...
  providers/vultr.py         NEW â€” one MaskProvider per remote model, source="vultr"
...
studio captures viewport renders â†’ `POST /masks/{asset}/compute` (multipart, `images[]`) â†’
cortex `registry.compute()` â†’ `vultr.py` provider â†’ `_vultr_request(task, png, params)` â†’
Vultr box runs the model â†’ returns small JSON â†’ provider projects onto parts via the shared
vertical-band heuristic â†’ `store.upsert(mask)` â†’ rail renders by archetype.

## Model lineup (starter)

| Route | Model | Provider key | name | category | archetype â†’ role | params |
|---|---|---|---|---|---|---|
| `/segment` | `nvidia/segformer-b4-finetuned-ade-512-512` | `part_segmentation_hq` | Part segmentation (HQ) | physics | categorical â†’ mask | â€” |
| `/sam` | SAM2 (or MobileSAM) | `sam_parts` | SAM parts | physics | categorical â†’ mask | â€” |
| `/clipseg` | `CIDAS/clipseg-rd64-refined` | `text_mask` | Text mask | physics | categorical â†’ mask | `prompt` (str) |
| `/depth` | `depth-anything/Depth-Anything-V2-Small-hf` | `depth` | Depth | artistic | scalar â†’ gradient | â€” |

`text_mask` pairs with the Gemini provider (Gemini names a part â†’ CLIPSeg masks it). Niche
...
- **One FastAPI app**, routes: `GET /health`, `POST /segment`, `POST /sam`, `POST /clipseg`,
  ...
  query/JSON params (CLIPSeg `prompt`). They return **small JSON only â€” never raw images**.
  ...
  - `/depth` returns `{grid: [[...]], min, max, h, w}` â€” a downsampled scalar grid.
...
- **Config** resolves like the HF/Gemini keys: `KINETIX_VULTR_URL` / `KINETIX_VULTR_TOKEN` env, with
  gitignored `.vultr_url` / `.vultr_token` repo-root file fallbacks.
- `available()` â†’ `True` only when a URL is configured **and** a short-TTL-cached `/health` ping
...
`hf.py` and `vultr.py` both map "ordered segment labels â†’ part_ids by vertical band". Lift
...
- `vultr/README.md` documents: create GPU instance â†’ `git pull` the `vultr/` folder â†’
  `bootstrap.sh` â†’ copy public IP into local `.vultr_url`, token into `.vultr_token`.
- **Burn rate stated loudly:** a ~$0.50â€“1/hr GPU â‡’ ~100â€“200 hrs of the $100 credit; an always-on
  box would silently drain it. README flags "DESTROY THE INSTANCE AFTER EACH SESSION" up top.

## Testing

- `tests/test_masks_vultr.py` â€” providers with `_vultr_request` mocked (mirrors
  ...
  `available()` gating (no URL â†’ unavailable; `compute` raises `RuntimeError`).
- `tests/test_vultr_serve.py` â€” `serve.py` via FastAPI `TestClient` with models monkeypatched:
