# kinetiX Studio â€” Bake Workbench Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Turn `studio/` into a live IDE where you import a 3D mesh, the real Python cortex bakes it, and the Properties panel shows the real mass / centre-of-mass / materials â€” in the approved austere visual system.

**Architecture:** A FastAPI HTTP bridge (`studio/server.py`) wraps `cortex` and answers the browser; the React studio calls it via `api.ts` and renders `PAP`/`Verdict` JSON (mirrors of the frozen `contracts.py`). Request/response (Option A) â€” every button is one real cortex call.
...
**Reference:** the approved visual mockup is `.superpowers/brainstorm/.../refined.html` â€” it is the exact markup/CSS source for the UI components and the icon `<symbol>` sheet. Design rules: no glows, no LED dots, no gradients; hairlines + Geist/Geist Mono; sage only where meaningful; gate colors the only saturated colors.
...
- `studio/server.py` â€” FastAPI app: `/health`, `/bake`, (stubs `/validate` `/repair` for M2). Owns an in-memory `{asset_id: PAP}` registry + `WorldModel`.
- `tests/test_studio_server.py` â€” TestClient tests.

**Frontend (`studio/src/`):**
- `theme.css` â€” design tokens (`:root`), base element styles, the inline SVG icon `<symbol>` sheet exported as a React component `Icons`.
- `api.ts` â€” typed fetch wrappers + TS types mirroring `contracts.py` (`PAP`, `Verdict`, `Gate`, `MaterialGuess`).
- `App.tsx` â€” REPLACE current body with the IDE shell: menubar, 3-col top (Assets/Viewport/Properties), full-width node-editor slot (mounts existing `ConstraintGraph`).
- `AssetsPanel.tsx` â€” import (file/drop), asset list, thumbnails, selection.
- `Viewport.tsx` â€” Three.js render of the selected mesh + CoM marker.
- `Properties.tsx` â€” render a `PAP`.
- `GateStack.tsx` â€” flat gate strip from a `Verdict` (used in M2; built now, fed fixtures).
- `Brand.tsx` â€” the text-only kinetiX wordmark.

**Do NOT touch:** `ConstraintGraph.tsx` (Fara), `RerunViewer.tsx` (kept for later streaming).

---

## Task 0: Integration â€” bring `cortex` onto the studio branch
...
helpers (tmp_path, mock_ue5_actor, mock_ue5_scene) to `tests/helpers.py` â€” copy them from
...
## Task 1: Backend skeleton â€” `/health`
...
app = FastAPI(title="kinetiX Studio backend")
app.add_middleware(
...
## Task 2: Backend `/bake` â€” real composition bake â†’ PAP
...
If `bake_asset`'s signature differs, read `cortex/bake/__init__.py` and adapt the call â€”
...
- [ ] **Step 2: Create `theme.css` â€” copy the `:root` tokens + base rules from the mockup**
...
- [ ] **Step 3: Create `Icons.tsx` â€” the custom SVG symbol sheet as a React component**
...
export function IconDefs() {
  return (
    <svg width="0" height="0" style={{ position: 'absolute' }} aria-hidden>
      {/* paste every <symbol id="i-..."> from refined.html here */}
    </svg>
  )
}
...
- [ ] **Step 4: Create `Brand.tsx` (text-only wordmark)**

```tsx
// studio/src/Brand.tsx â€” renders the text-only kinetiX wordmark
export function Brand() {
  return (
    <div className="brand">      <span className="word">kinetiX</span>
    </div>
  )
}
```
...
## Task 4: `api.ts` â€” typed client mirroring the contract
...
## Task 5: `Properties.tsx` â€” render a PAP (with a component test)
...
export function Properties({ pap }: { pap: PAP | null }) {
  if (!pap) return <section className="pane props"><header><div className="t"><Icon name="com"/><span>Properties â€” PAP</span></div></header><div className="body" style={{padding:16,color:'var(--ink3)'}}>Select an asset.</div></section>
  const f3 = (n:number)=> (n>=0?'':'') + n.toFixed(2).replace(/^0/,'')
  return (
    <section className="pane props">
      <header><div className="t"><Icon name="com"/><span>Properties â€” PAP</span></div><span className="mono" style={{fontSize:10,color:'var(--ink4)'}}>baked</span></header>
      <div className="body">
        <div className="psec">
          ...
          <div className="prop"><span className="k"><Icon name="seal"/>watertight</span><span className="v muted">{pap.geometry.watertight?`yes Â· ${pap.geometry.convex_parts} parts`:'no'}</span></div>
        ...
        <div className="psec" style={{borderBottom:'none'}}>
          <div className="label">Materials <span style={{color:'var(--ink4)',textTransform:'none'}}>â€” AI-guessed</span></div>
          <div className="mats">
            {pap.semantics.materials.map(m=>(
              <div className="mat" key={m.part}><span className="ml"><span className="swatch" style={{background:SWATCH[m.mat]??'#666'}}/>{m.part}</span><span className="mr">{m.mat} Â· {m.conf.toFixed(2)}</span></div>
            ))}
          </div>
...
## Task 6: `Viewport.tsx` â€” Three.js render of the selected mesh

**Files:**
- Create: `studio/src/Viewport.tsx`

- [ ] **Step 1: Implement** â€” a Three.js canvas that loads an `OBJLoader` from a Blob URL of the selected file, frames it, lights it flatly (no glow), dark `--inset` background; overlays a CoM dot when a `com` prop is passed. Port the viewport `<header>` + `.stage` chrome (crop marks, axis label) from the mockup around the canvas.
...
export function Viewport({ file, name }: { file: File | null; name: string }) {
  ...
  return (
    <section className="pane viewport">
      <header><div className="t"><Icon name="aperture"/><span>Viewport â€” {name || 'â€”'}</span></div><span className="mono" style={{fontSize:10,color:'var(--ink4)'}}>orbit Â· frame</span></header>
...
## Task 7: `AssetsPanel.tsx` â€” import, list, thumbnails, selection

**Files:**
- Create: `studio/src/AssetsPanel.tsx`

- [ ] **Step 1: Implement** â€” port the `<section class="pane assets">` markup; the list is driven by an `assets` prop `{id, name, file, pap?, status}`; clicking calls `onSelect`; the dropzone + an `<input type=file>` calls `onImport(file)`. Thumbnail: a small faceted `<svg>` placeholder now (a later WP swaps in a real offscreen Three render).
          ...
          {assets.map(a=>(
            <div key={a.id} className={`asset${a.id===selected?' sel':''}`} onClick={()=>onSelect(a.id)}>
              ...
              <div className="meta"><div className="nm">{a.name}</div>
                <div className="sub">{a.status==='baking'?'bakingâ€¦':a.status==='error'?<span style={{color:'var(--fail)'}}>bake failed</span>:a.pap?`${a.pap.semantics.cls} Â· ${a.pap.physical.mass_kg.toFixed(1)}kg`:''}</div></div>
            </div>
          ))}
          <label className="dropzone">
            <Icon name="import"/><div className="dz">Drop a mesh to bake</div><div className="dz2">.obj Â· .glb Â· .fbx</div>
...
## Task 8: `GateStack.tsx` â€” flat gate strip from a Verdict
...
- [ ] **Step 2: Run it to verify it fails** â€” `cd studio && npm test` â†’ FAIL (no GateStack).

- [ ] **Step 3: Implement** â€” port the `.gates` strip from the mockup; map `attempt.gates` to cells; status class by `ok/skipped`; format `value_m*100` cm; no dots/glow.
...
const cm = (m:number|null)=> m===null?'idle':`${m>=0?'+':'âˆ’'}${Math.abs(m*100).toFixed(1)} cm`
    ...
    <div className="gates">
      ...
      <div className="gflow">
        {order.map((name,i)=>{ const g=by.get(name); const c=g?cls(g,attempt.soft_cost):'idle'
          return (<><div key={name} className={`gate ${c}`}><Icon name={ICON[name]}/><span className="gn">{name}</span><span className="gv">{g?cm(g.value_m):'idle'}</span></div>{i<3 && <span className="chev">â€º</span>}</>) })}
      </div>
...
- [ ] **Step 4: Run the test to verify it passes** â€” `cd studio && npm test` â†’ PASS.
- [ ] **Step 5: Commit**
...
## Task 9: `App.tsx` shell â€” wire import â†’ bake â†’ Properties
...
import { Properties } from './Properties'
import { GateStack } from './GateStack'
import ConstraintGraph from './ConstraintGraph'   // Fara's â€” unchanged
  ...
  return (
    <div className="app">
      ...
      <div className="menubar">
        ...
        <div className="proj"><span className="dot"/><span className="mono">untitled.wdf</span><span style={{color:'var(--ink4)'}}>Â·</span>{assets.length} assets</div>
      </div>
...
```
Open the studio, **Import mesh** â†’ pick a `.obj` â†’ the asset appears "bakingâ€¦" then resolves;
...
git -c user.name="zajalist" -c user.email="saracensaray@gmail.com" commit -m "studio: IDE shell â€” import -> bake -> Properties wired to real cortex"
```
...
- [ ] **Step 1: Backend suite** â€” `./.venv/Scripts/python.exe -m pytest tests/test_studio_server.py -q` â†’ PASS.
- [ ] **Step 2: Frontend suite** â€” `cd studio && npm test` â†’ PASS (Properties, GateStack).
- [ ] **Step 3: Manual end-to-end** â€” import the bronze figure mesh â†’ Properties shows ~48 kg, high CoM, bronze/stone; no glows/LED-dots anywhere; Geist fonts; text-only wordmark top-left.
...
- M2 (live `/validate` + `/repair` + Inspector controls) is a follow-up plan â€” the GateStack + backend stubs already exist for it.
