...
> ## âš ï¸ DESTROY THE INSTANCE AFTER EACH SESSION
> A single GPU instance bills **by the hour**. At ~$0.50â€“1/hr the $100 credit lasts **~100â€“200
> hours** â€” but only if you **destroy** (not just stop) the instance when you're done. An
> always-on box silently drains the whole credit in under a week. Spin up â†’ work â†’ destroy.
...
JSON only** â€” never images.

| Route | Model (default, override via env) | Returns |
|---|---|---|
| `GET /health` | â€” | `{ok, device, loaded, models}` |
| `POST /segment` | `nvidia/segformer-b4-finetuned-ade-512-512` | `[{label, score}, â€¦]` |
| `POST /sam` | `facebook/sam-vit-base` | `[{label, score}, â€¦]` (labels synthesised by area) |
...
1. **Create a GPU instance** on Vultr (Cloud GPU â†’ cheapest single NVIDIA GPU, Ubuntu 22.04).
...
3. **Run bootstrap** (installs CUDA torch + deps, prints a token, launches the server):
   ...
   > **Two firewalls to open port 8001:** the Vultr *edge* firewall group (portal â†’ Network â†’
...
4. **Wire the studio to the box** â€” back on your machine, in the repo root:
   ```bash
   echo "http://<INSTANCE_PUBLIC_IP>:8001" > .vultr_url
   echo "<TOKEN_PRINTED_BY_BOOTSTRAP>"     > .vultr_token
   ```
   (Both are gitignored. You can also set `KINETIX_VULTR_URL` / `KINETIX_VULTR_TOKEN` env vars.)
...
Vultr gates Cloud GPU behind a support request â€” a fresh account that tries to deploy a `vcg-*`
...
2. On the box: `git clone â€¦ && cd plumb/vultr && bash bootstrap_cpu.sh`
...
CPU (tens of seconds). Swap to the GPU box (`bootstrap.sh`) once Vultr enables it â€” nothing on the
...
- **Swap weights:** set `VULTR_SEGMENT_MODEL` / `VULTR_SAM_MODEL` / `VULTR_CLIPSEG_MODEL` /
  `VULTR_DEPTH_MODEL` before launching â€” no code change.
- **Add a model:** add an `infer_<task>()` + a route in `serve.py`, then register a matching
  ...
  and MCP â€” no other wiring.

## When you're done

**Destroy the instance** in the Vultr console (Server â†’ Destroy). Stopping is not enough on some
plans â€” destroy to stop billing. Delete `.vultr_url` locally so the rail greys the masks back out.
