"""
studio/uasset.py â€” headless Unreal â†’ glTF conversion for the bake.
...
``Content/kinetiXImport/`` (where UE's boot scan registers it), export, then remove
the copy. Only mesh *geometry* is needed â€” material/texture references that don't
...
Config (graceful: absent => callers degrade to "import .obj/.glb/.stl"):
  KINETIX_UE_CMD      path to UnrealEditor-Cmd.exe (auto-detected if unset)
  KINETIX_UE_PROJECT  path to a host .uproject whose Content we stage imports into
"""
...
IMPORT_FOLDER = "kinetiXImport"  # staged under <project>/Content/<IMPORT_FOLDER>/
...
def ue_cmd() -> str | None:
    ...
    env = os.environ.get("KINETIX_UE_CMD")
...
def ue_project() -> str | None:
    """Path to the host .uproject we stage imports into (env ``KINETIX_UE_PROJECT``)."""
    env = os.environ.get("KINETIX_UE_PROJECT")
...
def _export_script(stems: list[str], out_dir: str) -> str:
    ...
    return (
        ...
        "    if mesh is None:\n"
        "        unreal.log_warning('kinetiX missing ' + obj); continue\n"
        ...
        "    unreal.log('kinetiX exported %s ok=%s' % (stem, ok))\n"
        "unreal.log('kinetiX DONE')\n"
    )
...
def convert_uassets(assets: dict[str, str], timeout: int = 600) -> dict[str, str | None]:
    ...
    ``.glb`` for it (missing asset, non-static-mesh, or export error) â€” the caller
    surfaces that per file rather than failing the whole batch.
    """
    cmd, proj = ue_cmd(), ue_project()
    if not (cmd and proj):
        raise RuntimeError("Unreal not configured (set KINETIX_UE_CMD and KINETIX_UE_PROJECT)")

    project_dir = os.path.dirname(proj)
    staging = os.path.join(project_dir, "Content", IMPORT_FOLDER)
    os.makedirs(staging, exist_ok=True)
    out_dir = tempfile.mkdtemp(prefix="kinetix_glb_")
    ...
    try:
        ...
        script_path = os.path.join(out_dir, "_kinetix_export.py")
        ...
        # path with KINETIX_UE_GPU=1 (e.g. if a future export needs baked textures).
        argv = [cmd, proj, "-run=pythonscript", f"-script={script_path}",
                "-unattended", "-nopause", "-nosplash", "-stdout"]
        if os.environ.get("KINETIX_UE_GPU", "0") != "1":
...
def convert_uasset(name: str, path: str, timeout: int = 600) -> str | None:
    """Convert a single ``.uasset`` â†’ ``.glb`` path (or ``None`` on failure)."""
