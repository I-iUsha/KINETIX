...
function statusLabel(s: Asset['status']): string {
  return s === 'queued' ? 'queued'
    : s === 'converting' ? 'converting Â· Unreal'
    : s === 'baking' ? 'decomposing Â· physics'
    : s === 'ok' ? 'ready'
    : s === 'error' ? 'failed'
    : s
}
...
export function Stage({ assets, settings, setSettings, onAddFiles, onBake, onUpdateAsset, onRemove, onEnter, ueAvailable }: {
  ...
  // queued OR previously-errored meshes can (re)bake â€” both need a file
  ...
  return (
    <div className="stage-screen">
      <div className="stage-head">
        <span className="wordmark">kinetiX</span>
          ...
          <section className="upanel">
            ...
            <div className="upanel-bd">
              <div className={`stage-drop${dragging ? ' on' : ''}`}
                ...
                <div className="sd-2 mono">.obj Â· .glb Â· .stl{ueAvailable ? ' Â· .uasset' : ''}</div>
                <div className="sd-3">a .gltf auto-pulls its .bin + textures (incl. nested folders)</div>
                <button className="sd-folder" onClick={(e) => { e.stopPropagation(); folderRef.current?.click() }}>Choose a folderâ€¦</button>
          ...
          <section className="upanel">
            ...
            <div className="upanel-bd stage-settings">
              ...
              <div className="ss-note">{ueAvailable
                ...
                : '.uasset needs Unreal (set KINETIX_UE_CMD + KINETIX_UE_PROJECT).'}</div>
            </div>
          </section>
        </div>
                    ...
                    <div className="sq-main">
                      <div className="sq-top">
                        ...
                        <button className="sq-x" title="Remove from queue"
                          onClick={() => onRemove(a.id)}>âœ•</button>
                      </div>
                      ...
                      <div className="sq-meta mono">
                        {a.status === 'ok' && a.pap
                          ? `${a.profile ?? settings.profile} Â· ${a.pap.parts?.length ?? a.pap.geometry.convex_parts} masks Â· ${a.pap.physical.mass_kg.toFixed(1)} kg`
                          : a.status === 'error'
      ...
      <div className="stage-foot">
        {ready > 0 && toBake > 0 && !baking && (
          <button className="stage-enter ghost" onClick={onEnter}>Enter editor Â· {ready} â†’</button>
        )}
        <button className="stage-enter" disabled={baking || (toBake === 0 && ready === 0)}
          ...
          {baking ? 'Bakingâ€¦'
            ...
            : ready > 0 ? `Enter editor Â· ${ready} asset${ready > 1 ? 's' : ''} â†’`
