...
/** The data every kinetiX node carries (shared by the canvas + inspector). */
export type kinetiXData = {
  kind: NodeKind
  op: NodeOp
  label: string
  sub?: string
...
export type EvalContext = { scene: SceneState; inputs: PortValue[]; data: kinetiXData }
...
export function evaluateGraph(
  ...
  const data = (n: Node) => n.data as kinetiXData
