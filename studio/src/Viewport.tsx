...
export function Viewport({ name, file, extras, pap, pos, rot = [0, 0, 0], scale = 1, verdict, status, swept, onDropFiles, capping, onApplyCap, onExitCap, busy, capResult, onCapAgain, onDismissCap, onCaptureReady }: {
  ...
  useEffect(() => {
    ...
    const capture = async () => {
      ...
      return image && image.size > 0 ? [image] : []
    }
  ...
  const requestCompute = (key: string) => {
    const prov = catalog.find((p) => p.key === key)
    if (!prov) return
    if (key === 'text_mask' || prov.source === 'gemini') {
      setMaskHint('')
      setMaskQuestion({ key, name: prov.name, required: key === 'text_mask', wantsHint: key === 'text_mask' })
      return
    }
    void runCompute(key)
  }
