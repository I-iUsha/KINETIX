...
function Ports({ data }: { data: kinetiXData }) {
  return (
    <>
      {data.accepts?.length ? (
        <Handle
...
// ── node renderers ────────────────────────────────────────────────────────────
function AssetNode({ data }: NodeProps<Node<kinetiXData>>) {
  return (
...
function MeasureNode({ id, data }: NodeProps<Node<kinetiXData>>) {
...
function LawNode({ id, data }: NodeProps<Node<kinetiXData>>) {
  const res = useContext(ResultsContext).get(id)
...
function FieldNode({ data }: NodeProps<Node<kinetiXData>>) {
  return (
...
function VerdictNode({ id, data }: NodeProps<Node<kinetiXData>>) {
  const res = useContext(ResultsContext).get(id)
  const status = res?.status ?? 'idle'
  const color = STATUS_COLOR[status]
  return (
 ...
 * improve feedback).
 */
function rejectHint(src?: kinetiXData, tgt?: kinetiXData): string | null {
...
export default function ConstraintGraph({
  ...
  const selectedNodes = useMemo(
    () =>
      selectedIds.map((id) => {
        const d = nodes.find((n) => n.id === id)?.data as kinetiXData | undefined
  ...
  const selectedNode = (nodes.find((n) => n.id === selectedId) ?? null) as Node<kinetiXData> | null
    ...
    () =>
      edges
        ...
        .map((e) => {
          ...
          const data = src?.data as kinetiXData | undefined
  ...
  const selectedEdge = useMemo<EdgeInfo | null>(() => {
    ...
    if (!e) return null
    const src = nodes.find((n) => n.id === e.source)?.data as kinetiXData | undefined
    const tgt = nodes.find((n) => n.id === e.target)?.data as kinetiXData | undefined
    return {
      id: e.id,
  ...
  // Shared accessor: a node's kinetiXData by id, for the connection rules.
  const getData = useCallback<GetNodeData>(
    (id) => getNode(id)?.data as kinetiXData | undefined,
    [getNode],
  )
  ...
  const onConnectEnd = useCallback(
    (_: MouseEvent | TouchEvent, state: FinalConnectionState) => {
      ...
      const from = (state.fromNode?.data ?? undefined) as kinetiXData | undefined
      const to = (state.toNode?.data ?? undefined) as kinetiXData | undefined
  ...
  const onDrop = useCallback(
    (e: React.DragEvent) => {
      ...
      const kind = e.dataTransfer.getData('application/kinetix-node-kind') as NodeKind | ''
      const op = kind ? DEFAULT_OP_BY_KIND[kind] : e.dataTransfer.getData('application/kinetix-node')
  ...
  const onSetHard = useCallback(
    (id: string, hard: boolean) => {
      takeSnapshot()
      setNodes((nds) =>
        nds.map((n) => (n.id === id ? { ...n, data: { ...(n.data as kinetiXData), hard } } : n)),
      )
    },
    [setNodes, takeSnapshot],
  )
