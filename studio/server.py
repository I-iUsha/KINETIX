...
class Placement(BaseModel):
    ...
    # topples it â€” only real tilt/slope does. False â†’ the anchored pedestal model.
...
def _persist_geometry(asset_id: str, mesh_path: str, parts: list[dict]) -> None:
    ...
    later (the in-memory PAP doesn't keep parts geometry). Best-effort â€” never fails a bake."""
    try:
...
def _hf_status() -> dict:
    ...
    Kept inline (not via the providers package) so /health stays import-light â€” the
...
async def semantics(
    ...
    for image in images[:4]:
        if image.content_type not in (None, "image/png"):
            raise HTTPException(status_code=415, detail="render images must be PNG")
        data = await image.read()
        if not data:
            raise HTTPException(status_code=422, detail="render image is empty")
        if len(data) > 8 * 1024 * 1024:
            raise HTTPException(status_code=413, detail="each render image must be 8 MB or smaller")
...
def _maybe_decimate(path: str, target_faces: int) -> str:
    ...
    setting is a hint, never a hard failure â€” we never block a bake on it).
    """
    try:
        import trimesh
        ...
        return path  # decimation unsupported / failed â†’ bake the original
...
async def bake(
    ...
    non-self-contained formats â€” a ``.gltf`` references its geometry in a ``.bin`` (and
    ...
    if is_uasset(fn):
        # Unreal .uasset â†’ headless glTF first (then bake the glb like any other mesh).
        ...
        if not ue_status()["available"]:
            raise HTTPException(
                status_code=503,
                detail="Unreal not configured (set KINETIX_UE_CMD + KINETIX_UE_PROJECT) â€” import .obj/.glb/.stl instead",
            )
    ...
    except Exception as e:  # bad mesh / decomposition failure â€” surface, don't crash
        msg = str(e)
        if fn.lower().endswith(".gltf") and ("No such file" in msg or ".bin" in msg):
            msg = (".gltf needs its external files â€” drop the .gltf together with its "
...
async def convert(files: list[UploadFile] = File(...)) -> dict:
    ...
    ``/bake_cached`` (fast â€” the conversion already happened). Non-uasset files are
    ignored here (the UI bakes those directly through ``/bake``).
    """
    from studio.uasset import convert_uassets, is_uasset, ue_status

    if not ue_status()["available"]:
        raise HTTPException(status_code=503, detail="Unreal not configured (KINETIX_UE_CMD + KINETIX_UE_PROJECT)")
...
async def open_wdf(doc: UploadFile = File(...)) -> dict:
    ...
    the scene's ``laws`` are the constraints. The studio renders both â€” no bake
...
# --------------------------------------------------------------------------- #
# Projects â€” save/open a project bundling the .wdf semantics + the 3D models.
# --------------------------------------------------------------------------- #
...
async def export_repaired(
    ...
    """Bake a repaired placement transform into a self-contained GLB download."""
    import numpy as np
    import trimesh
    ...
    fn = mesh.filename or "asset.glb"
    work = tempfile.mkdtemp(prefix="kinetix_repaired_")
    path = os.path.join(work, os.path.basename(fn))
    with open(path, "wb") as f:
        f.write(await mesh.read())
    for extra in extras:
        if extra.filename:
            with open(os.path.join(work, os.path.basename(extra.filename)), "wb") as f:
                f.write(await extra.read())

    try:
        tf = Transform.model_validate(json.loads(transform))
        ...
        x, y, z, w = (float(v) for v in tf.quat)
        scale = np.diag([float(tf.scale[0]), float(tf.scale[1]), float(tf.scale[2]), 1.0])
        matrix = (
            trimesh.transformations.translation_matrix(tf.pos)
            @ trimesh.transformations.quaternion_matrix([w, x, y, z])
            @ scale
        )
        scene.apply_transform(matrix)
        payload = scene.export(file_type="glb")
    except Exception as e:
        raise HTTPException(status_code=422, detail=f"GLB export failed: {e}") from e

    stem = os.path.splitext(os.path.basename(fn))[0]
    headers = {"Content-Disposition": f'attachment; filename="{stem}_repaired.glb"'}
...
# --------------------------------------------------------------------------- #
# Masks (design 2026-05-31) â€” the extensible semantic-overlay system. The UI
...
async def compute_mask(
    ...
    ``text_mask`` ``prompt``). 404 unknown asset Â· 503 provider unavailable/needs key Â· 502 compute error.
    """
    ...
    try:
        ...
        # studio) stay responsive â€” otherwise a single compute freezes every request.
        mask = await run_in_threadpool(masks.compute, asset, provider_key)
    except KeyError as e:
        raise HTTPException(status_code=404, detail=str(e)) from e
    except (RuntimeError, ValueError) as e:
        # unavailable provider / missing key / needs images â†’ 503 (configuration), else 502
...
# --------------------------------------------------------------------------- #
# Placement distributions â€” the studio captures demonstrated placements; the MCP
...
def swept(req: SweptReq) -> dict:
    ...
    Real geometry from ``cortex.bake_profiles.door.swept_volume`` â€” the union of
    door poses from 0Â° to ``range_deg`` about the hinge axis. Returns the wedge as
    ...
    except Exception as e:  # degenerate joint / mesh â€” surface, don't crash
